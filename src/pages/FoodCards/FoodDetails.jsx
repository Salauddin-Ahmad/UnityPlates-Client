...
const FoodDetails = () => {
  ...
  return (
    <div className="mb-8 relative">
      {foodDetails ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 mx-auto border rounded-md px-2 shadow-lg shadow-stone-600 text-xl">
          <div>
            <img className="w-full rounded-lg" src={foodDetails.image} alt={foodDetails.foodName} />
          </div>
          <div className="px-2 lg:px-4 space-y-6 mt-2">
            ...
            {/* Request Button */}
            <button className="bg-blue-500 text-white px-10 py-3 rounded-lg hover:bg-blue-600" onClick={handleRequestButtonClick}>Request</button>
          </div>
        </div>
      ) : (
        <p>Loading...</p>
